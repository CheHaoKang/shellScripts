...
if test -e $out_folder/flex_root/da80-target_files-signed_tmp.zip
then
  echo "Already have temp Signed Target-files"
else
  mkdir -p $out_folder/flex_root
  cp $out_folder/da80-target_files-signed.zip $out_folder/flex_root/da80-target_files-signed_tmp.zip
fi

cd $out_folder/flex_root
unzip da80-target_files-signed_tmp.zip FLEX/flex_$region/cci.prop
...
zip da80-target_files-signed_tmp.zip ./FLEX/flex_$region/cci.prop

echo "Transfer to ext4 image"

mkdir -p Signed_Image

$ANDROID_BUILD_TOP/build/tools/releasetools/img_from_target_files -f $region da80-target_files-signed_tmp.zip ./Signed_Image/$region-image.zip
