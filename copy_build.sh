...
export preVersion=$(($((`echo $cm_build_branch | sed -r 's/WB\.[a-zA-Z0-9]+\.[a-zA-Z0-9]+\.[0]*//g'`))-1))
...
#need to check
./repo forall -p -c 'git log remotes/korg/WB.0.0.$preVersion..$cm_build_branch | grep BugID' > Check-inSummary.txt
